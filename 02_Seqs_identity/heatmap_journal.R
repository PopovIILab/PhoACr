...
p1<-data_general %>%
  ...
  theme_classic()+
  scale_fill_gradient2(name="Nucleotide<br>sequence<br>identity<br>",
                      low = "white",
                      mid = "lightblue",
                      high = "darkblue",
                      na.value = "white",
  ...
  scale_y_discrete(labels = function(labels) gsub("^PK", "", labels),
                   position = "left") +
...
metadata <- read.table('../01_Phylogenetics/metadata/metadata.tsv', sep='\t', header=T)
...
ggsave("images/HM.png", plot = combined, width=16, height=12, dpi=600)