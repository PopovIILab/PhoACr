...
data <- data %>%
  mutate(
    Primer = ifelse(str_detect(line, "^Primer name"), str_extract(line, "(?<=Primer name ).+"), NA)
  ) %>%
...
data <- data %>%
  mutate(
    Sequence = ifelse(str_detect(line, "^\\s*Sequence:"), str_extract(line, "(?<=Sequence: )[^ ]+"), NA)
  ) %>%
...
data <- data %>%
  mutate(
    Forward_mismatches = ifelse(str_detect(line, "hits forward strand"), as.numeric(str_extract(line, "\\d+(?= mismatches)")), NA),
    Reverse_mismatches = ifelse(str_detect(line, "hits reverse strand"), as.numeric(str_extract(line, "\\d+(?= mismatches)")), NA)
  )

# Filter rows to keep only lines where mismatches are recorded
mismatch_data <- data %>%
  filter(!is.na(Forward_mismatches) | !is.na(Reverse_mismatches)) %>%
...
p1 <- metadata %>% 
  ggplot()+
  geom_tile(aes(x=Axis_A, fill=Sequence_id, 
                y=factor(Name, 
                         level=c("Bat coronavirus",
                                  "Shrew coronavirus",
                                  "Rodent coronavirus",
  ...
  scale_fill_manual(breaks = c("Humans",
                               "Bats",
                               "Birds",
                               "Rodents",
                               "Pigs",
  ...
  scale_fill_manual(breaks = c("Alphacoronavirus",
                               "Betacoronavirus",
                               "Deltacoronavirus",
                               "Gammacoronavirus"),
                    values = c("#FFF8DC",
  ...
  scale_x_discrete(labels=c("a" = "GenBank ID",
                            "b" = "CoV<br>genus",
                            "c" = "Host"))+
...
PCR <- read.table("mismatch_df.csv", sep = ",", header = T)%>%
  pivot_longer(cols=-c(Sequence), 
               names_to="Primers", values_to="Mismatch")

p2 <- PCR %>% 
  ggplot()+
  geom_tile(aes(x=Primers, fill=Mismatch, y=factor(Sequence, 
                                                   level=c("NC_048212.1",
                                                           "NC_046955.1",
                                                           "NC_046954.1",
                                                           "NC_034440.1",
  ...
  scale_fill_manual(name="Mismatches",
                    breaks=c("Four", 
                             "No_hit",
                             "Two",
                             "Three",
                    ...
                    values=c("#CD5555",
                             ...
                             "#A2CD5A",
                             "#458B00"))+
  scale_x_discrete(labels=c("aCovbat1_Forward_mismatches" = "PrimA F",
  ...
  theme(axis.line = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank(),
        axis.text.x = element_markdown(angle=55, 
                                       hjust = 1, 
                                       size = 14),
...
ggsave("images/HM.png", plot = combined, width=14, height=12, dpi=600)  
