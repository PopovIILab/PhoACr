...
pacman::p_load(ggtree, ggimage, ggtext, phangorn, ggplot2, treeio, ggnewscale, viridis, phytools, patchwork)
...
tree_fig <- ggtree(tree) %<+% metadata + 
  ...
  geom_tiplab(
    aes(
      label = Full.Name,
      fontface = ifelse(grepl("^PQ", Full.Name), "bold", "plain")  # Bold if starts with "PQ"
    ),
    align=TRUE,
    color="black",
    geom="label",
...
tree_fig <- gheatmap(tree_fig, meta.loc, width=0.05, offset=1.5, color="black", font.size=4, colnames_offset_y = -0.05) + 
  scale_fill_manual(breaks = c("Russia",
...
tree_fig <- gheatmap(tree_fig, meta.host, width=0.05, offset=1.7, color="black", font.size=4, colnames_offset_y = -0.05) + 
  scale_fill_manual(breaks = c("Homo sapiens",
                               "Nyctalus noctula",
                               "Nyctalus velutinus",
                               "Pipistrellus kuhlii",
                               ...
                               "Bat",
                               "microbat",
                               "Rattus norvegicus", #Грызун
                               "Mustela vison", #Норка
                               "Sorex araneus", #Землеройка
                               "Suncus murinus", #Землеройка
                               "pig",
                               NA),
                    labels = c("*Homo sapiens*",
                               "*Nyctalus noctula*",
                               "*Nyctalus velutinus*",
                               "*Pipistrellus kuhlii*",
                               "*Pipistrellus nathusii*",
                               ...
                               "Pig*",
                               "NA"),
                    values = c("#D2B48C", # "Homo_sapiens"
                               "#00FF7F", # "Nyctalus_noctula"
                               "#008B45", # "Nyctalus_velutinus"
                               "#FFFF00", # "Pipistrellus_kuhlii"
                               "#EEEE00", # "Pipistrellus_nathusii"
                               "#8B8B00", # "Pipistrellus_pygmaeus"
                               "#8B475D", # "Myotis_brandtii"
                               "#8B636C", # "Myotis_dasycneme"
                               "#CD6889", # "Myotis_daubentonii"
                               ...
                               "#36648B", # "Rhinolophus_ferrumequinum"
                               "#4F94CD", # "Triaenops_afer"
                               "#5CACEE", # "Hipposideros_larvatus"
                               "#63B8FF", # "Desmodus_rotundus"
                               "#838B83", # "Bat"
                               "#C1CDC1", # "Microbat"
                               "#00868B", # "Rattus_norvegicus"
                               "#668B8B", # "Mustela_vison"
                               "#FFC125", # "*Sorex araneus*"
                               "#8B4500", # "*Suncus murinus*"
                               "#EEE9E9"),# "Pig"
                    na.translate = TRUE,
                    na.value = "white",
                    name="Host") +
...
tree_fig <- gheatmap(tree_fig, meta.year, width=0.05, offset=1.9, color="black", font.size=4, colnames_offset_y = -0.05) + 
  scale_fill_viridis(option="D", name="Year", discrete = TRUE, na.translate = TRUE)

# Save the fig

ggsave('images/tree.png', tree_fig, width=20, height=16, dpi=600)